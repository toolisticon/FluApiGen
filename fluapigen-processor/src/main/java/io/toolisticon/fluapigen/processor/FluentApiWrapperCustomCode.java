...
public class FluentApiWrapperCustomCode {
    ...
    @CustomCodeMethod(FluentApi.class)
    public static FluentApiInterfaceWrapper[] getFluentApiInterfaces(FluentApiWrapper wrapper) {
        ...
        FluentApiInterfaceWrapper[] fluentApiInterfaces = new FluentApiInterfaceWrapper[filterResult.size()];

        int i = 0;
        for (TypeElement typeElement : filterResult) {
            fluentApiInterfaces[i] = FluentApiInterfaceWrapper.wrap(typeElement);
            i++;
        }
    ...
    @CustomCodeMethod(FluentApi.class)
    public static FluentApiInterfaceWrapper getRootFluentApiInterface(FluentApiWrapper wrapper) {

        return  FluentApiInterfaceWrapper.wrap(wrapper.rootInterfaceAsTypeMirrorWrapper().getTypeElement());
    ...
    public static boolean validate(FluentApiWrapper wrapper) {
        ...
        for (FluentApiInterfaceWrapper fluentApiInterface : wrapper.getFluentApiInterfaces()) {
